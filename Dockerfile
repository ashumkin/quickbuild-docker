...
RUN wget 'http://www.pmease.com/artifact?file=quickbuild-6.1.3.tar.gz&buildId=2964' -O quickbuild.tar && tar -zxvf quickbuild.tar -C /opt
...
RUN wget 'http://dev.mysql.com/get/Downloads/Connector-J/mysql-connector-java-5.1.38.tar.gz' -O mysql-connector-java-5.1.38.tar.gz \
    ...
    && cp -av mysql-connector-java-5.1.38-bin.jar /opt/quickbuild-6.1.3/plugins/com.pmease.quickbuild.libs/mysql-connector-java-5.1.38.jar
# add config files
ADD *.properties /opt/quickbuild-6.1.3/conf/
# Expose the default QB port
EXPOSE 8810
# start quickbuild
ENTRYPOINT /opt/quickbuild-6.1.3/bin/server.sh console
